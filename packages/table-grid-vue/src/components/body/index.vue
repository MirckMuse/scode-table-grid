...
import MergedCell from "./MergedCell.vue";
...
function resetGridTemplateRows() {
	gridTemplateRows.value = Array.from(
		tableState.get_row_heights(dataSource.value),
	);
...
const bodyCenterColKeys = computed(
	() => tableState?.last_center_col_keys ?? [],
);
...
const bodyCenterStyle = computed<StyleValue>(() => {
	...
	const paddingLeft = tableState
		.get_col_state()
		.get_reduce_width(last_left_col_keys);
...
const commonRowProps = reactive({
	...
	isMergedCell: (colKey: ColKey, rowKey: RowKey) =>
		tableState.is_merged_cell(rowKey, colKey),
});
...
const updateCellSizes = (mutationsList: MutationRecord[]) => {
	...
	const cells = Array.from(
		tableBodyRef.value?.querySelectorAll(
			".s-table-body-cell:not(.s-table-body-cell__merged)",
		) ?? [],
	) as HTMLElement[];
...
const $scrollObserver = new IntersectionObserver(() => refresh(), {
	threshold: 0,
	rootMargin: "50%",
});
...
onMounted(() => {
	if (tableBodyInnerRef.value) {
		$cellResize.observe(tableBodyInnerRef.value, {
			childList: true,
			subtree: true,
		});
	}
